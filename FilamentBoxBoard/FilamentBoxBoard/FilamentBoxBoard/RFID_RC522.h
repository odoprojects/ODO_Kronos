...
enum MIFARE_Misc {
 	MF_ACK					= 0x0A,		// The MIFARE Classic uses a 4 bit ACK/NAK. Any other value than 0xA is NAK.
 	MF_KEY_SIZE				= 0x06			// A Mifare Crypto1 key is 6 uint8_ts.
};
...
void PCD_WriteRegisters(uint8_t reg, uint8_t count, uint8_t *values);
...
static const void *GetStatusCodeName(enum StatusCode code);
enum PICC_Type PICC_GetType(uint8_t sak);
// old function used too much memory, now name moved to flash; if you need char, copy from flash to memory
//const char *PICC_GetTypeName(uint8_t type);
static const void *PICC_GetTypeName(enum PICC_Type type);
